...
export default {
  ...
  methods: {
    load () {
      axios.get('http://localhost:8029/api/periode/').then(res => {
    ...
    add (instance, row, value) {
      axios.get('http://localhost:8029/api/periode/').then(res => {
        ...
        console.log(index[0])
        axios.post('http://localhost:8029/api/periode/', {
          id: index[0]
        })
      })
    },
    update (instance, cell, columns, row, value) {
      axios.get('http://localhost:8029/api/periode/').then(res => {
        ...
        index[columns] = value
        console.log(index)
        axios.put('http://localhost:8029/api/periode/' + dataold[0], {
          id: index[0],
          nama: index[1],
          last_update: index[4]
        })
    ...
    delete (instance, row) {
      axios.get('http://localhost:8029/api/periode/').then(res => {
        var index = Object.values(res.data[row])
        axios.delete('http://localhost:8029/api/periode/' + index[0])
